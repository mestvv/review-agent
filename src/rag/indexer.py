...
# Паттерны секций
SECTION_PATTERNS = {
    "abstract": [r"^(?:abstract|аннотация|резюме)\b"],
    "introduction": [
    ...
    "results": [r"^(?:\d+\.?\s*)?(?:results?|результаты?)\b"],
    "discussion": [r"^(?:\d+\.?\s*)?(?:discussion|обсуждение)\b"],
    "conclusion": [r"^(?:\d+\.?\s*)?(?:conclusions?|выводы?|заключение)\b"],
    "references": [
